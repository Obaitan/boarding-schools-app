...
class SubmitDocsForm(FlaskForm):
    ...
    docs = MultipleFileField(
        "Documents", validators=[DataRequired(), MultiFileAllowed(["pdf"])]
    )
    recaptcha = RecaptchaField()
...
class BoardingSchoolsForm(FlaskForm):
    ...
    logo = FileField(
        "School Logo", validators=[DataRequired(), FileAllowed(["jpg", "jpeg", "png"])]
    )
    badge = FileField(
        "School Badge", validators=[DataRequired(), FileAllowed(["jpg", "jpeg", "png"])]
    )
...
class ImageForm(FlaskForm):
    ...
    schoolName = QuerySelectField(
        label=u"School Name",
        validators=[DataRequired()],
        query_factory=query_factory,
        get_pk=get_pk,
    )
    schoolPix = MultipleFileField(
        "School Forms",
        validators=[DataRequired(), MultiFileAllowed(["jpg", "jpeg", "png"])],
    )
...
class EditImageForm(FlaskForm):
    ...
    submit = SubmitField("Update Pictures")
...
class DocumentsForm(FlaskForm):
    ...
    appForm = FileField(
        "School Application Form", validators=[Optional(), FileAllowed(["pdf"])]
    )
    feesForm = FileField(
        "School Fees Form", validators=[Optional(), FileAllowed(["pdf"])]
    )
    submit = SubmitField("Submit Documents")
...
class NewsForm(FlaskForm):
    def validate_title(self, title_to_check):
        ti = News.query.filter_by(title=title_to_check.data).first()
        if ti:
            raise ValidationError(
    ...
    def validate_excerpt(self, excerpt_to_check):
        ex = News.query.filter_by(excerpt=excerpt_to_check.data).first()
        if ex:
            raise ValidationError(
    ...
    def validate_article(self, article_to_check):
        art = News.query.filter_by(article=article_to_check.data).first()
        if art:
            raise ValidationError(
    ...
    excerpt = TextAreaField(
        "Article Excerpt", validators=[DataRequired(), Length(max=350)]
    )
