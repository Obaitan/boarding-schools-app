...
def single_school(name, name2):
    ...
    return render_template(
        "pages/school-template-one.html",
        college=college,
        images=images,
        thumbs=thumbs,
        docs=docs,
...
def apply_form(name):
    ...
    if request.method == "POST":
        if form.validate() == False:
            flash(
                'Please meet the requirement for each field and click "CONTINUE APPLICATION".',
                category="danger",
            )
            ...
            msg = Message(
                subject,
                sender="webforms@boardingschoolsexperts.com",
...
def schools_form():
    ...
    if request.method == "POST":
        if form.validate() == False:
            flash(
                'Please meet the requirement for each field and click "SUBMIT FORM" again.',
                category="danger",
            )
            ...
            flash(
                "Your form has been submited! We will be in touch with you shortly",
                category="success",
            )
...
def agents_form():
    ...
    if request.method == "POST":
        if form.validate() == False:
            flash(
                'Please provide valid information for all required fields before clicking "SUBMIT FORM" again.',
                category="danger",
            )
...
def contact():
    ...
    if request.method == "POST":
        if form.validate() == False:
            flash(
                'Please meet the requirement for each field and click "SEND" again.',
                category="danger",
            )
...
def submit_docs():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            flash(
                f"Please Meet The Requirements of The Form Fields And Submit Again.",
                category="danger",
            )
...
def new_article():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            if image:
                ...
                new_post = News(
                    ...
                    imagePath=os.path.abspath(
                        os.path.join(app.config["UPLOAD_FOLDER"], imageName)
                    ),
                )
                db.session.add(new_post)
                db.session.commit()
                image.save(os.path.join(app.config["UPLOAD_FOLDER"], imageName))
                flash(f"News Article Published Successfully!.", category="success")
                return redirect(request.url)
            else:
            ...
            flash(
                f"Please Meet The Requirements For The Form Before Submiting It.",
                category="danger",
            )
            return render_template("admin/add-article.html", form=form)
    else:
...
def new_school_info():
    ...
    if s:
        flash(
            f"{form.schoolName.data} Already Has An Entry! Go To Schools Table To Edit It.",
            category="danger",
        )
        return redirect(request.url)
    else:
                ...
                if logo and badge:
                    ...
                    new_school = Schools(
                        ...
                        logoPath=os.path.abspath(
                            os.path.join(app.config["UPLOAD_FOLDER"], logoName)
                        ),
                        badge=badgeName,
                        badgePath=os.path.abspath(
                            os.path.join(app.config["UPLOAD_FOLDER"], badgeName)
                        ),
                    ...
                    badge.save(os.path.join(app.config["UPLOAD_FOLDER"], badgeName))
                    flash(
                        f"Information for {schoolName} Has Been Saved. Please Upload Images And Forms Next.",
                        category="success",
                    )
                    return redirect(request.url)
                else:
                    flash(
                        "Please Upload Images (PNG / JEPG / JPG) For Both Of The Image Fields",
                        category="danger",
                    )
                    return render_template(
                        "admin/add-school-information.html", form=form
                    )
            else:
                flash(
                    f"Please Meet The Requirements For The Form Before Submiting It.",
        ...
        if request.method == "POST":
            if form.validate_on_submit():
                ...
                for pix in form.schoolPix.data:
                    ...
                    schImage = Images(
                        schoolName=name,
                        schoolPix=schPixName,
                        schoolPixPath=os.path.abspath(
                            os.path.join(app.config["UPLOAD_FOLDER"], schPixName)
                        ),
                    )
                    db.session.add(schImage)
                    pix.save(os.path.join(app.config["UPLOAD_FOLDER"], schPixName))
                db.session.commit()
                flash(
                    f"Images For {name} Have Been Uploaded Successfully.",
                    category="success",
                )
                return redirect(request.url)
            else:
...
def new_school_thumbnails():
    ...
    if t:
        flash(
            f"{form.name.data} Already Has An Entry! Click On Its Badge To Edit It.",
            category="danger",
        )
        return redirect(request.url)
    else:
            ...
            if form.validate_on_submit():
                ...
                for image in form.vidPix.data:
                    ...
                    vidImage = Thumbnails(
                        name=name,
                        vidPix=vidPixName,
                        vidPixPath=os.path.abspath(
                            os.path.join(app.config["UPLOAD_FOLDER"], vidPixName)
                        ),
                    )
                    db.session.add(vidImage)
                    image.save(os.path.join(app.config["UPLOAD_FOLDER"], vidPixName))
                db.session.commit()
                flash(
                    f"YouTube Images For {name} Have Been Uploaded Successfully.",
                    category="success",
                )
                return redirect(request.url)
            else:
...
def new_school_forms():
    form = DocumentsForm()
    school_name = Documents.query.filter_by(schoolName=form.schoolName.data).first()
    if school_name:
        flash(
            f"{form.schoolName.data} Already Has An Entry! Click On Its Badge To Edit It.",
            category="danger",
        )
        return redirect(request.url)
    else:
            ...
            if form.validate_on_submit():
                ...
                if af and ff:
                    ...
                    forms = Documents(
                        schoolName=name,
                        appForm=afName,
                        appFormsPath=os.path.abspath(
                            os.path.join(app.config["UPLOAD_FOLDER"], afName)
                        ),
                        feesForm=ffName,
                        feesFormsPath=os.path.abspath(
                ...
                try:
                    ...
                    db.session.commit()
                    flash(
                        f"Form(s) for {name} Have Been Uploaded Successfully.",
                        category="success",
                    )
                    if af:
                        af.save(os.path.join(app.config["UPLOAD_FOLDER"], afName))
                    if ff:
                        ff.save(os.path.join(app.config["UPLOAD_FOLDER"], ffName))
                except:
                    db.session.rollback()
                    flash(
                        "There Was An Error While Uploading Form(s). Please Try Again Or Contact Administrator.",
                        category="danger",
                    )
                return redirect(request.url)
            else:
                flash(
                    "Please Meet The Requirements For The Form Before Submiting It.",
                    category="danger",
                )
...
@app.route("/admin/schools/edit-info/<string:name>", methods=["GET", "POST"])
@login_required
def edit_school_info(name):
    school = Schools.query.filter_by(schoolName=name).first()
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash(
                f"Information for {school.schoolName} Has Been Updated.",
                category="success",
            )
            return redirect(url_for("admin_schools"))
        else:
            flash(f"Error! Please Try Again.", category="danger")
            return redirect(request.url)
    else:
        return render_template("admin/edit-school-info.html", form=form, school=school)


# @app.route('/admin/schools/edit-images/<string:name>', methods=['GET', 'POST'])
# @login_required
# def edit_school_images(name):
#     school = Schools.query.filter_by(schoolName=name).first()
#     images = Images.query.filter_by(schoolName=name).all()
#     form = EditImageForm(obj=images)
#     if request.method == "POST":
#         if form.validate_on_submit():
#             name = form.schoolName.data
#             for pix in form.schoolPix.data:
...
#             db.session.commit()
#             flash(f'Images For {name} Have Been Updated.', category='success')
#             return redirect(url_for('admin_schools'))
#         else:
#             flash(f'Error! Please Try Again.', category='danger')
#             return redirect(request.url)
#     else:
#         return render_template('admin/edit-school-images.html', form=form, images=images, school=school)


# @app.route('/admin/schools/edit-thumbnails/<string:name>', methods=['GET', 'POST'])
# @login_required
# def edit_school_thumbnails(name):
#     school = Schools.query.filter_by(schoolName=name).first()
#     thumbs = Thumbnails.query.filter_by(name=name).all()
#     form = EditImageForm(obj=images)
#     if request.method == "POST":
...
def add_country():
    ...
    if flag:
        ...
        if place:
            flash(
                f"{name} Already Exists! Please Enter A New Country.", category="danger"
            )
            return redirect(url_for("country"))
        elif emblem:
            flash(
                f"{flagName} Has Already Been Used For Another Country!",
                category="danger",
            )
            return redirect(url_for("country"))
        else:
            new_country = Country(
                ...
                flagPath=os.path.abspath(
                    os.path.join(app.config["UPLOAD_FOLDER"], flagName)
                ),
            )
